...
public class StreamTextHolder extends TextMessageHolder {
    ...
    private void refreshUI(TUIMessageBean msg, int position) {
        ...
        if (hasRiskContent) {
            setRiskContent(itemView.getResources().getString(com.tencent.qcloud.tuikit.deskchat.R.string.chat_risk_send_message_failed_alert));
        }
        ...
        if (isForwardMode || isReplyDetailMode || !textMessageBean.isSelf()) {
            int otherTextColorResId = TUIThemeManager.getAttrResId(msgBodyText.getContext(),
                    com.tencent.qcloud.tuikit.deskchat.R.attr.chat_other_msg_text_color);
            int otherTextColor = msgBodyText.getResources().getColor(otherTextColorResId);
            msgBodyText.setTextColor(otherTextColor);
        } else {
            int selfTextColorResId = TUIThemeManager.getAttrResId(msgBodyText.getContext(), com.tencent.qcloud.tuikit.deskchat.R.attr.chat_self_msg_text_color);

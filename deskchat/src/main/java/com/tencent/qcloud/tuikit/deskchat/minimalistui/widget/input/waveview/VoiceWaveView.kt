package com.tencent.qcloud.tuikit.deskchat.minimalistui.widget.input.waveview
...
class VoiceWaveView @JvmOverloads constructor(
     ...
     * 跳动模式
     */
    var deskWaveMode: WaveMode = WaveMode.UP_DOWN

    /**
     * 线条样式
     */
    var deskLineType: LineType = LineType.BAR_CHART
    ...
    init {
        attrs?.let {
            val typedArray = context.theme.obtainStyledAttributes(
                attrs,
                R.styleable.TxVoiceWaveView, 0, 0
            )

            lineWidth = typedArray.getDimension(R.styleable.TxVoiceWaveView_lineWidth, 5f)
            lineSpace = typedArray.getDimension(R.styleable.TxVoiceWaveView_lineSpace, 5f)
            duration = typedArray.getInt(R.styleable.TxVoiceWaveView_duration, 200).toLong()
            showGravity = typedArray.getInt(R.styleable.TxVoiceWaveView_android_gravity, Gravity.LEFT or Gravity.BOTTOM)
            lineColor = typedArray.getInt(R.styleable.TxVoiceWaveView_lineColor, Color.BLUE)
            val mode = typedArray.getInt(R.styleable.TxVoiceWaveView_deskWaveMode, 0)
            when (mode) {
                0 -> deskWaveMode = WaveMode.UP_DOWN
                1 -> deskWaveMode = WaveMode.LEFT_RIGHT
            }

            val lType = typedArray.getInt(R.styleable.TxVoiceWaveView_deskLineType, 0)
            when (lType) {
                0 -> deskLineType = LineType.BAR_CHART
                1 -> deskLineType = LineType.LINE_GRAPH
            }
    ...
    fun start() {
        ...
        isStart = true
        if (deskWaveMode == WaveMode.UP_DOWN) {
            ...
            valueAnimator.start()
        } else if (deskWaveMode == WaveMode.LEFT_RIGHT) {
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        for (i in waveList.indices) {
            ...
            if (deskLineType == LineType.BAR_CHART) {
                paintLine?.let {
                    canvas?.drawLine(
                        startX,
                        startY,
            ...
            if (deskLineType == LineType.LINE_GRAPH) {
                if (i == 0) {
